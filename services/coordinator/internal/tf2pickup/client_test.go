...
func TestActiveGamesAndForceEnd(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/api/frontress/v1/games/"+matchID+"/result":
			var body struct {
				RedScore int             `json:"redScore"`
				BluScore int             `json:"bluScore"`
				Aborted  bool            `json:"aborted"`
				Players  []requestPlayer `json:"players"`
			}
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Fatal(err)
			}
			if body.RedScore != 5 || body.BluScore != 3 || body.Aborted || len(body.Players) != 1 || body.Players[0].Team != "red" {
				t.Errorf("result body = %#v", body)
			}
			fmt.Fprintf(w, `{"externalMatchId":%q,"map":"koth_product_final","matchGroup":7,"maxPlayers":24,"state":"ended","createdAt":%q,"readyAt":%q,"startedAt":%q,"score":{"red":5,"blu":3},"players":[],"server":null}`, matchID, createdAt.Format(time.RFC3339), readyAt.Format(time.RFC3339), readyAt.Format(time.RFC3339))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	finished, err := client.ReportResult(context.Background(), matchID, wire.MatchResult{
		MatchID: matchID, RedScore: 5, BluScore: 3,
		Players: []wire.AssignedPlayer{{SteamID: "76561198000000001", Team: wire.TeamRed}},
	})
	if err != nil {
		t.Fatal(err)
	}
	if finished.State != "ended" || finished.RedScore != 5 || finished.BluScore != 3 {
		t.Fatalf("ReportResult = %#v", finished)
	}
}
