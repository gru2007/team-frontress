...
func (c *Client) ReportResult(ctx context.Context, matchID string, matchResult wire.MatchResult) (mm.BackendGame, error) {
	players := make([]requestPlayer, 0, len(matchResult.Players))
	for _, player := range matchResult.Players {
		players = append(players, encodePlayer(player))
	}
	body := map[string]any{
		"redScore": matchResult.RedScore,
		"bluScore": matchResult.BluScore,
		"aborted":  matchResult.Aborted,
		"players":  players,
	}
	var result response
	path := "/api/frontress/v1/games/" + url.PathEscape(matchID) + "/result"
...
func encodePlayers(players []wire.AssignedPlayer) []requestPlayer {
	...
	for _, player := range players {
		out = append(out, encodePlayer(player))
	}
	return out
}

func encodePlayer(player wire.AssignedPlayer) requestPlayer {
	team := "red"
	if player.Team == wire.TeamBlu {
		team = "blu"
	}
	return requestPlayer{SteamID: player.SteamID, Name: player.Name, Team: team}
}
