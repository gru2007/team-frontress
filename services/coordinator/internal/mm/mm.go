...
// MatchAssignment returns the coordinator-owned view of a running match for a
// game-server GC session. It deliberately does not expose Match or its mutable
// slices: the GC is another consumer of matchmaking state, not a second owner.
func (m *Matchmaker) MatchAssignment(id string) (*wire.Assignment, bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
	mt, ok := m.matches[id]
	if !ok || mt.state == msOver || mt.Server == nil {
		return nil, false
	}
	return cloneAssignment(m.assignmentLocked(mt, wire.TeamUnassigned, false)), true
}
