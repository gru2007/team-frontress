...
type CMsgProtoBufHeader struct {
	...
	ClientSteamId   *uint64 `protobuf:"fixed64,1,opt,name=client_steam_id,json=clientSteamId" json:"client_steam_id,omitempty"`      // SteamID of the client sending this, typically set in all client originated messages.
	ClientSessionId *int32  `protobuf:"varint,2,opt,name=client_session_id,json=clientSessionId" json:"client_session_id,omitempty"` // SessionID of the client on the CM
	// Source appId for inter-gc messages
	SourceAppId *uint32 `protobuf:"varint,3,opt,name=source_app_id,json=sourceAppId" json:"source_app_id,omitempty"` // appId of source GC message sender
	// Job routing (may be set on client or inter-server messages)
	JobIdSource   *uint64 `protobuf:"fixed64,10,opt,name=job_id_source,json=jobIdSource,def=18446744073709551615" json:"job_id_source,omitempty"` // JobID that sent this message
	JobIdTarget   *uint64 `protobuf:"fixed64,11,opt,name=job_id_target,json=jobIdTarget,def=18446744073709551615" json:"job_id_target,omitempty"` // The target job which is expected to be waiting on this message
	TargetJobName *string `protobuf:"bytes,12,opt,name=target_job_name,json=targetJobName" json:"target_job_name,omitempty"`                      // the type of job to start when this message is received
	Eresult       *int32  `protobuf:"varint,13,opt,name=eresult,def=2" json:"eresult,omitempty"`                                                  // For response jobs, the corresponding eresult
	ErrorMessage  *string `protobuf:"bytes,14,opt,name=error_message,json=errorMessage" json:"error_message,omitempty"`                           // Optionally an error message in case of failure. Mostly used for debugging purpose.
	// Where did this message originally enter the system? From a client, from another GC, etc
	GcMsgSrc *GCProtoBufMsgSrc `protobuf:"varint,200,opt,name=gc_msg_src,json=gcMsgSrc,enum=GCProtoBufMsgSrc" json:"gc_msg_src,omitempty"`
...
const file_steammessages_proto_rawDesc = "" +
	"\n" +
	"\x13steammessages.proto\x1a google/protobuf/descriptor.proto\"\xd8\x03\n" +
	...
	"\rerror_message\x18\x0e \x01(\tR\ferrorMessage\x120\n" +
	"\n" +
	"gc_msg_src\x18\xc8\x01 \x01(\x0e2\x11.GCProtoBufMsgSrcR\bgcMsgSrc\x12.\n" +
	"\x13gc_dir_index_source\x18\xc9\x01 \x01(\rR\x10gcDirIndexSource:\n" +
	"\x80\xa6\x1d\x80\x02\x88\xa6\x1d\x80\b*4\n" +
	"\x10GCProtoBufMsgSrc\x12 \n" +
	"\x1cGCProtoBufMsgSrc_Unspecified\x10\x00:C\n" +
...
var file_steammessages_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
...
var file_steammessages_proto_depIdxs = []int32{
	...
	2, // 1: key_field:extendee -> google.protobuf.FieldOptions
	3, // 2: msgpool_soft_limit:extendee -> google.protobuf.MessageOptions
	3, // 3: msgpool_hard_limit:extendee -> google.protobuf.MessageOptions
