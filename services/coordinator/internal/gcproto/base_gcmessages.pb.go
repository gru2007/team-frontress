...
const (
	EGCBaseMsg_k_EMsgGCSystemMessage    EGCBaseMsg = 4001 // broadcast announcements from the GC
	EGCBaseMsg_k_EMsgGCReplicateConVars EGCBaseMsg = 4002 // GC => client
	EGCBaseMsg_k_EMsgGCConVarUpdated    EGCBaseMsg = 4003 // GC => client
	...
	EGCBaseMsg_k_EMsgGCServerAvailable          EGCBaseMsg = 4506 // send from a dedicated server when its ready
	EGCBaseMsg_k_EMsgGCClientConnectToServer    EGCBaseMsg = 4507 // sent to a client to connect to a server
	EGCBaseMsg_k_EMsgGCGameServerInfo           EGCBaseMsg = 4508 // send from a dedicated server for server address information
	EGCBaseMsg_k_EMsgGCError                    EGCBaseMsg = 4509 // sent from GC to client telling client about an error
	EGCBaseMsg_k_EMsgGCReplay_UploadedToYouTube EGCBaseMsg = 4510 // client => GC
	EGCBaseMsg_k_EMsgGCLANServerAvailable       EGCBaseMsg = 4511 // send from a listen server when its ready
)
...
type CGCStorePurchaseInit_LineItem struct {
	...
	ItemDefId           *uint32                `protobuf:"varint,1,opt,name=item_def_id,json=itemDefId" json:"item_def_id,omitempty"`                                 // DefIndex of the item to purchase
	Quantity            *uint32                `protobuf:"varint,2,opt,name=quantity" json:"quantity,omitempty"`                                                      // quantity to purchase
	CostInLocalCurrency *uint32                `protobuf:"varint,3,opt,name=cost_in_local_currency,json=costInLocalCurrency" json:"cost_in_local_currency,omitempty"` // cost in cents of the local currency the user thinks he should pay (if you change this update item_price_t!)
	PurchaseType        *uint32                `protobuf:"varint,4,opt,name=purchase_type,json=purchaseType" json:"purchase_type,omitempty"`                          // is this a regular purchase? a rental? maps to ECartItemType
...
type CMsgGCStorePurchaseInit struct {
	state         protoimpl.MessageState           `protogen:"open.v1"`
	Country       *string                          `protobuf:"bytes,1,opt,name=country" json:"country,omitempty"`    // Country the purchase is being made from (obtained from Steam)
	Language      *int32                           `protobuf:"varint,2,opt,name=language" json:"language,omitempty"` // Client's language
	Currency      *int32                           `protobuf:"varint,3,opt,name=currency" json:"currency,omitempty"` // Currency the purchase is in (obtained from Steam)
...
type CMsgGCStorePurchaseInitResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Result        *int32                 `protobuf:"varint,1,opt,name=result" json:"result,omitempty"`            // Result of the operation
	TxnId         *uint64                `protobuf:"varint,2,opt,name=txn_id,json=txnId" json:"txn_id,omitempty"` // Transaction ID of the new transaction
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgSystemBroadcast struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       *string                `protobuf:"bytes,1,opt,name=message" json:"message,omitempty"` // the message to display on the client
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgClientGoodbye struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Reason        *GCGoodbyeReason       `protobuf:"varint,1,opt,name=reason,enum=GCGoodbyeReason" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *CMsgClientGoodbye) GetReason() GCGoodbyeReason {
	...
	return GCGoodbyeReason_GCGoodbyeReason_GC_GOING_DOWN
}
...
type CSOEconGameAccountClient struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	AdditionalBackpackSlots *uint32                `protobuf:"varint,1,opt,name=additional_backpack_slots,json=additionalBackpackSlots,def=0" json:"additional_backpack_slots,omitempty"` // the number of backpack slots this user has on top of DEFAULT_NUM_BACKPACK_SLOTS
	...
	MatchmakingRankedBanExpiration                   *uint32 `protobuf:"varint,18,opt,name=matchmaking_ranked_ban_expiration,json=matchmakingRankedBanExpiration" json:"matchmaking_ranked_ban_expiration,omitempty"`                                   // player can't matchmake in ranked groups until this time
	MatchmakingRankedLowPriorityExpiration           *uint32 `protobuf:"varint,24,opt,name=matchmaking_ranked_low_priority_expiration,json=matchmakingRankedLowPriorityExpiration" json:"matchmaking_ranked_low_priority_expiration,omitempty"`         // MM low priority in ranked groups until
	MatchmakingRankedBanLastDuration                 *uint32 `protobuf:"varint,25,opt,name=matchmaking_ranked_ban_last_duration,json=matchmakingRankedBanLastDuration" json:"matchmaking_ranked_ban_last_duration,omitempty"`                           // How long the current ranked ban's total duration was set for
	MatchmakingRankedLowPriorityLastDuration         *uint32 `protobuf:"varint,26,opt,name=matchmaking_ranked_low_priority_last_duration,json=matchmakingRankedLowPriorityLastDuration" json:"matchmaking_ranked_low_priority_last_duration,omitempty"` // How long the current ranked ban's total duration was set for
	MatchmakingCasualBanExpiration                   *uint32 `protobuf:"varint,27,opt,name=matchmaking_casual_ban_expiration,json=matchmakingCasualBanExpiration" json:"matchmaking_casual_ban_expiration,omitempty"`                                   // player can't matchmake in casual groups until this time
	MatchmakingCasualLowPriorityExpiration           *uint32 `protobuf:"varint,28,opt,name=matchmaking_casual_low_priority_expiration,json=matchmakingCasualLowPriorityExpiration" json:"matchmaking_casual_low_priority_expiration,omitempty"`         // MM low priority in casual groups until
	MatchmakingCasualBanLastDuration                 *uint32 `protobuf:"varint,29,opt,name=matchmaking_casual_ban_last_duration,json=matchmakingCasualBanLastDuration" json:"matchmaking_casual_ban_last_duration,omitempty"`                           // How long the current casual ban's total duration was set for
	MatchmakingCasualLowPriorityLastDuration         *uint32 `protobuf:"varint,30,opt,name=matchmaking_casual_low_priority_last_duration,json=matchmakingCasualLowPriorityLastDuration" json:"matchmaking_casual_low_priority_last_duration,omitempty"` // How long the current casual ban's total duration was set for
	...
	MatchmakingLastCasualExcessiveReportsAutoBanTime *uint32 `protobuf:"varint,34,opt,name=matchmaking_last_casual_excessive_reports_auto_ban_time,json=matchmakingLastCasualExcessiveReportsAutoBanTime" json:"matchmaking_last_casual_excessive_reports_auto_ban_time,omitempty"` // Last time we were banned for excessive reports in casual
	MatchmakingLastCompExcessiveReportsAutoBanTime   *uint32 `protobuf:"varint,35,opt,name=matchmaking_last_comp_excessive_reports_auto_ban_time,json=matchmakingLastCompExcessiveReportsAutoBanTime" json:"matchmaking_last_comp_excessive_reports_auto_ban_time,omitempty"`       // Last time we were banned for excessive reports in comp
	unknownFields                                    protoimpl.UnknownFields
	sizeCache                                        protoimpl.SizeCache
}
...
type CMsgIncrementKillCountAttribute struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	KillerSteamId  *uint64                `protobuf:"varint,1,opt,name=killer_steam_id,json=killerSteamId" json:"killer_steam_id,omitempty"` // Kyle says: we could make these both fixed32!
	...
	IncrementValue *uint32                `protobuf:"varint,5,opt,name=increment_value,json=incrementValue" json:"increment_value,omitempty"` // How much to increment the score by
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
type CMsgApplyStrangeCountTransfer struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ToolItemId     *uint64                `protobuf:"varint,1,opt,name=tool_item_id,json=toolItemId" json:"tool_item_id,omitempty"` // item id of the xfer tool
...
type CMsgApplyStrangePart struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	StrangePartItemId *uint64                `protobuf:"varint,1,opt,name=strange_part_item_id,json=strangePartItemId" json:"strange_part_item_id,omitempty"` // which part are we "inserting"?
	ItemItemId        *uint64                `protobuf:"varint,2,opt,name=item_item_id,json=itemItemId" json:"item_item_id,omitempty"`                        // what strange item are we inserting this new part into?
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
type CMsgApplyStrangeRestriction struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	StrangePartItemId *uint64                `protobuf:"varint,1,opt,name=strange_part_item_id,json=strangePartItemId" json:"strange_part_item_id,omitempty"` // which restriction item are we "inserting"?
	ItemItemId        *uint64                `protobuf:"varint,2,opt,name=item_item_id,json=itemItemId" json:"item_item_id,omitempty"`                        // what strange item are we inserting this new part into?
	StrangeAttrIndex  *uint32                `protobuf:"varint,3,opt,name=strange_attr_index,json=strangeAttrIndex" json:"strange_attr_index,omitempty"`      // which slot did the user select for application?
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
type CMsgApplyUpgradeCard struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	UpgradeCardItemId *uint64                `protobuf:"varint,1,opt,name=upgrade_card_item_id,json=upgradeCardItemId" json:"upgrade_card_item_id,omitempty"` // which card are we attaching?
	SubjectItemId     *uint64                `protobuf:"varint,2,opt,name=subject_item_id,json=subjectItemId" json:"subject_item_id,omitempty"`               // what item are we applying this new card to?
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
type CSOEconItemAttribute struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DefIndex      *uint32                `protobuf:"varint,1,opt,name=def_index,json=defIndex" json:"def_index,omitempty"`
	Value         *uint32                `protobuf:"varint,2,opt,name=value" json:"value,omitempty"` // DEPRECATED -- see value_bytes
...
type CSOEconItem struct {
	...
	ContainsEquippedState   *bool                   `protobuf:"varint,17,opt,name=contains_equipped_state,json=containsEquippedState" json:"contains_equipped_state,omitempty"` // DEPRECATED
	...
	ContainsEquippedStateV2 *bool                   `protobuf:"varint,19,opt,name=contains_equipped_state_v2,json=containsEquippedStateV2" json:"contains_equipped_state_v2,omitempty"` // will be set to true even if equipped_state is an empty array, meaning "unequipped from everything"
	unknownFields           protoimpl.UnknownFields
	sizeCache               protoimpl.SizeCache
}
...
type CMsgAdjustItemEquippedState struct {
	...
	NewSlot       *uint32                `protobuf:"varint,3,opt,name=new_slot,json=newSlot" json:"new_slot,omitempty"` // will be -1 if not equipped on this class any longer
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgStoreGetUserDataResponse struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	Result            *int32                 `protobuf:"varint,1,opt,name=result" json:"result,omitempty"`                                                   // Result of the call
	Currency          *int32                 `protobuf:"varint,2,opt,name=currency" json:"currency,omitempty"`                                               // Currency to display to the user
	Country           *string                `protobuf:"bytes,3,opt,name=country" json:"country,omitempty"`                                                  // Country the purchase is being made from (Send back in k_EMsgGCStorePurchaseInit)
	PriceSheetVersion *uint32                `protobuf:"fixed32,4,opt,name=price_sheet_version,json=priceSheetVersion" json:"price_sheet_version,omitempty"` // Version of the current price sheet on the GC
	// experiments
	ExperimentData      *uint64 `protobuf:"varint,5,opt,name=experiment_data,json=experimentData,def=0" json:"experiment_data,omitempty"` // top 32 bits = experiment id, bottom 32 bits = experiment group number
...
type CMsgUpdateItemSchema struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ItemsGame         []byte                 `protobuf:"bytes,1,opt,name=items_game,json=itemsGame" json:"items_game,omitempty"`                             // actual contents of items_game.txt (only used on dev)
	ItemSchemaVersion *uint32                `protobuf:"fixed32,2,opt,name=item_schema_version,json=itemSchemaVersion" json:"item_schema_version,omitempty"` // Version of the items_game.txt we're using
	ItemsGameUrl      *string                `protobuf:"bytes,3,opt,name=items_game_url,json=itemsGameUrl" json:"items_game_url,omitempty"`                  // HTTP URL where they can use to fetch the one we're using, if theirs is out of date and we don't send the contents
	Signature         []byte                 `protobuf:"bytes,4,opt,name=signature" json:"signature,omitempty"`                                              // signature of the schema (either the one at the CDN, or the bytes we are providing).  In certain branches (TF) this is required.
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
type CMsgUseItem struct {
	state                   protoimpl.MessageState `protogen:"open.v1"`
	ItemId                  *uint64                `protobuf:"varint,1,opt,name=item_id,json=itemId" json:"item_id,omitempty"`
	TargetSteamId           *uint64                `protobuf:"fixed64,2,opt,name=target_steam_id,json=targetSteamId" json:"target_steam_id,omitempty"` // 64-bit field left over from original message
...
type CMsgSelectPresetForClass struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ClassId       *uint32                `protobuf:"varint,1,opt,name=class_id,json=classId" json:"class_id,omitempty"`    // which class we're changing the selection on
	PresetId      *uint32                `protobuf:"varint,2,opt,name=preset_id,json=presetId" json:"preset_id,omitempty"` // which preset we want to change to
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgGCReportAbuse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TargetSteamId *uint64                `protobuf:"fixed64,1,opt,name=target_steam_id,json=targetSteamId" json:"target_steam_id,omitempty"` // who is the user accusing?
	Description   *string                `protobuf:"bytes,4,opt,name=description" json:"description,omitempty"`                              // in the user's own words
	Gid           *uint64                `protobuf:"varint,5,opt,name=gid" json:"gid,omitempty"`                                             // meaning depends on content type
	// If accusing a player:
	AbuseType   *uint32 `protobuf:"varint,2,opt,name=abuse_type,json=abuseType" json:"abuse_type,omitempty"`       // EAbuseReportType
	ContentType *uint32 `protobuf:"varint,3,opt,name=content_type,json=contentType" json:"content_type,omitempty"` // ECommunityContentType
...
type CMsgGCReportAbuseResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TargetSteamId *uint64                `protobuf:"fixed64,1,opt,name=target_steam_id,json=targetSteamId" json:"target_steam_id,omitempty"` // target to which this reply is in reference
	Result        *uint32                `protobuf:"varint,2,opt,name=result" json:"result,omitempty"`                                       // EResult
	ErrorMessage  *string                `protobuf:"bytes,3,opt,name=error_message,json=errorMessage" json:"error_message,omitempty"`        // Diagnostic error message (not localized, for debugging purposes only)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgGCIncrementKillCountResponse struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	KillerAccountId *uint32                `protobuf:"varint,1,opt,name=killer_account_id,json=killerAccountId" json:"killer_account_id,omitempty"` // name of the user who got the kill
	NumKills        *uint32                `protobuf:"varint,2,opt,name=num_kills,json=numKills" json:"num_kills,omitempty"`                        // number of kills (or: ubers released; or gifts given out; etc.)
	ItemDef         *uint32                `protobuf:"varint,3,opt,name=item_def,json=itemDef" json:"item_def,omitempty"`                           // id of the item in question
	LevelType       *uint32                `protobuf:"varint,4,opt,name=level_type,json=levelType" json:"level_type,omitempty"`                     // what sort of rank is this? (ie., kills, gifts, etc.) used for looking up strings
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
type CMsgGCRemoveStrangePart struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	ItemId               *uint64                `protobuf:"varint,1,opt,name=item_id,json=itemId" json:"item_id,omitempty"`                                               // ID of the item we're removing a part from
	StrangePartScoreType *uint32                `protobuf:"varint,2,opt,name=strange_part_score_type,json=strangePartScoreType" json:"strange_part_score_type,omitempty"` // score type counted by the strange part we want to remove -- it doesn't matter what slot this part is in or what item def it came from as each item can only have one counter of each type
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}
...
type CMsgGCRemoveUpgradeCard struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ItemId         *uint64                `protobuf:"varint,1,opt,name=item_id,json=itemId" json:"item_id,omitempty"`                         // ID of the item we're removing a part from
	AttributeIndex *uint32                `protobuf:"varint,2,opt,name=attribute_index,json=attributeIndex" json:"attribute_index,omitempty"` // the attribute index of the attribute we want to remove; this is only valid if the attribute is set to be user-customizable
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
type CMsgGCStorePurchaseCancel struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TxnId         *uint64                `protobuf:"varint,1,opt,name=txn_id,json=txnId" json:"txn_id,omitempty"` // Transaction ID for the the transaction
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgGCStorePurchaseCancelResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Result        *uint32                `protobuf:"varint,1,opt,name=result" json:"result,omitempty"` // Result of the operation
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgGCStorePurchaseFinalizeResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Result        *uint32                `protobuf:"varint,1,opt,name=result" json:"result,omitempty"`                  // Result of the operation
	ItemIds       []uint64               `protobuf:"varint,2,rep,name=item_ids,json=itemIds" json:"item_ids,omitempty"` // If successful, list of uint64's that represent the purchased items
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgGCBannedWordListRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	BanListGroupId *uint32                `protobuf:"varint,1,opt,name=ban_list_group_id,json=banListGroupId" json:"ban_list_group_id,omitempty"` // The group code to request the word list from (English, Chinese, etc)
	WordId         *uint32                `protobuf:"varint,2,opt,name=word_id,json=wordId" json:"word_id,omitempty"`                             // The most recent word ID that we want to request (the response will include this ID if present)
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
type CMsgApplyToolToItem struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ToolItemId    *uint64                `protobuf:"varint,1,opt,name=tool_item_id,json=toolItemId" json:"tool_item_id,omitempty"`          // which item tool are we using
	SubjectItemId *uint64                `protobuf:"varint,2,opt,name=subject_item_id,json=subjectItemId" json:"subject_item_id,omitempty"` // what item are we applying this tool to?
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type CMsgApplyToolToBaseItem struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	ToolItemId       *uint64                `protobuf:"varint,1,opt,name=tool_item_id,json=toolItemId" json:"tool_item_id,omitempty"`                   // which item tool are we using
	BaseitemDefIndex *uint32                `protobuf:"varint,2,opt,name=baseitem_def_index,json=baseitemDefIndex" json:"baseitem_def_index,omitempty"` // which base item def index are we applying this tool to?
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}
...
type CMsgFulfillDynamicRecipeComponent struct {
	state                 protoimpl.MessageState `protogen:"open.v1"`
	ToolItemId            *uint64                `protobuf:"varint,1,opt,name=tool_item_id,json=toolItemId" json:"tool_item_id,omitempty"`                               // which item tool are we using
	ConsumptionComponents []*CMsgRecipeComponent `protobuf:"bytes,2,rep,name=consumption_components,json=consumptionComponents" json:"consumption_components,omitempty"` // Items to consume
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}
...
type CMsgDeliverGiftResponseGiver struct {
	...
	ReceiverAccountName *string                `protobuf:"bytes,2,opt,name=receiver_account_name,json=receiverAccountName" json:"receiver_account_name,omitempty"` // will only be set if found and if the user wasn't specifically selected
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}
...
type CWorkshop_PopulateItemDescriptions_Request_ItemDescriptionsLanguageBlock struct {
	state         protoimpl.MessageState                                              `protogen:"open.v1"`
	Language      *string                                                             `protobuf:"bytes,1,opt,name=language" json:"language,omitempty"` // ICU name
...
const file_base_gcmessages_proto_rawDesc = "" +
	...
	"\x11CMsgClientGoodbye\x12(\n" +
	"\x06reason\x18\x01 \x01(\x0e2\x10.GCGoodbyeReasonR\x06reason\"=\n" +
	"\x11CMsgServerGoodbye\x12(\n" +
	"\x06reason\x18\x01 \x01(\x0e2\x10.GCGoodbyeReasonR\x06reason\"\x15\n" +
