...
// SelectedFromBits resolves CTFCasualMatchCriteria.selected_maps_bits. Valve
// assigns those bits by index in master_maps_list; All deliberately preserves
// that source order. An empty selection means the party accepts the configured
// match-group pool.
func SelectedFromBits(bits []uint32) []string {
	var selected []string
	for index, gameMap := range All {
		word := index / 32
		if word >= len(bits) || bits[word]&(uint32(1)<<uint(index%32)) == 0 {
			continue
		}
		selected = append(selected, gameMap.Name)
	}
	return selected
}
