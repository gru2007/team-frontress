...
**`serveme`** — reservations bought from a [serveme.tf](https://serveme.tf) fork:

```json
{ "kind": "serveme", "region": "eu",
  "base_url": "https://serveme.example.org", "api_key": "...",
  "reserve_mins": 120, "server_config_id": 0 }
```
...
or `max_match_secs` elapsed, and the server goes back.
