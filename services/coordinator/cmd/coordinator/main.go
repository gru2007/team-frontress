...
//
// It exposes the Valve GC protocol, owns party/queue/lobby state and delegates
// durable match/server lifecycle to tf2pickup-frontress.
package main
...
func run() error {
	...
	if !cfg.TF2Pickup.Enabled() {
		return errors.New("tf2pickup.base_url is required; the legacy RCON server lifecycle has been removed")
	}
	...
	handler := api.New(cfg, matchmaker, verifier, warEngine, records, log).Handler()
...
func writeDefaultConfig(w *os.File) error {
	...
	cfg.TF2Pickup.BaseURL = "http://tf2pickup:3000"
	cfg.TF2Pickup.Secret = "change-me"
