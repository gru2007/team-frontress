...
- an authoritative **Go Game Coordinator** behind Valve's native GC protocol;
- a persistent append-only **war event log**;
- durable match and server lifecycle in **tf2pickup-frontress**;
- Steam identity / session integration;
- an in-game HTML command board for the strategic layer;
- on-demand dedicated servers supplied through the lifecycle backend.
