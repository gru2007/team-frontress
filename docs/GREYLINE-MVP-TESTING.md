...
| `You are not permitted to join this server!` on a P2P battle | the Steam peer-to-peer handshake was dropped while the relay was still settling — the guest's console says `did not have the correct challenge` just before it. The menu retries the connect for `client_connect_deadline` and then gives the slot up rather than holding it |
